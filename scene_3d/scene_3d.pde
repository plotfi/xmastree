...
void draw() {
  ...
  background(#87CEFA);  // clear the screen to black
  ...
  camera (X, Y, Z, 0.0, 0.0, -1.0, 0.0, 1.0, 0.0);
  ...
  directionalLight(255, 255, 255, -0.7, -0.7, -Z);
...
void locamotive()
{
  ...
  rotate (radians(180), 1.0, 0.0, 0.0);
  rotateZ(radians(90));
  translate(0, -25, 0);
  rotateY(radians(-60));
  translate(2, 0, -4);
  ...
  translate (11.5, 5, -5.1);
  rotate (1.5, 1.0, 0.0, 0.0);
...
void cone (float radius, float height, int sides) {
  ...
  for (i = 0; i < sides; i++) {
    ...
    vertex (c[ii] * (i/sides), 0.0, s[ii] * (i/sides));
    vertex (c[i] * (i/sides), 0.0, s[i] * (i/sides));
    vertex (0.0, 0.0, 0.0);
    endShape();
  ...
  for (i = 0; i < sides; i++) {
    ...
    beginShape();
    normal (c[i], 0.0, s[i]);
    vertex (c[i] * radius*(i/sides), 0.0, s[i] * radius*(i/sides));
    vertex (c[i] * radius, height, s[i] * radius);
    normal (c[ii], 0.0, s[ii]);
    vertex (c[ii] * radius, height, s[ii] * radius);
    vertex (c[ii] * radius*(i/sides), 0.0, s[ii] * radius*(i/sides));
    endShape(CLOSE);
        if (i >= sides/2) break;
  }
}
...
// Draw a cylinder of a given radius, height and number of sides.
// The base is on the y=0 plane, and it extends vertically in the y direction.
void fullcone (float radius, float height, int sides) {
  int i,ii;
  float []c = new float[sides];
  float []s = new float[sides];
